...
proc addJob(els: seq[XmlNode], q: QueueXml, batches: seq[seq[
    string]]) = jobsQueueX.add (els, q, batches)
proc addJob(els: seq[VNode], q: QueueDom, batches: seq[seq[
    string]]) = jobsQueueV.add (els, q, batches)
...
proc reachedBufSize(s: auto, sz: int, bufsize: int): bool = (len(s) *
    gluePadding + sz) > bufsize
...
proc doQueryAll(els: auto, q: QueueXml | QueueDom, batches: seq[seq[
    string]]): Future[void] {.async.} =
